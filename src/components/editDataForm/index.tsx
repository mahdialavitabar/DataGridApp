...
import {rowAtom} from "@/components/dataTable";
...
export const EditDataForm = ({cancelOnClick}:any) => {
    const schema = object({
    ...
    const [rowData]= useAtom(rowAtom)

    const {id, age ,email,fullName,mobile} = rowData as any
    const {mutate} = useDynamicMutation({
        url: `https://62bfe35ec134cf51cec58701.mockapi.io/api/crud/users/${id}`,method:"put",
        onSuccess: () => {
